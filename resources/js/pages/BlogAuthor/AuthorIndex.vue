<template>
    <AppSectionHeader title="Authors" :bread-crumb="breadCrumb">
        <template #right>
            <AppButton
                v-if="auth.user && auth.user.permissions.includes('Blog: Author - Create')"
            ...
            <tbody>
                <AppDataTableRow v-for="item in authors.data" :key="item.id">
                    ...
                    <AppDataTableData>
                        <!-- edit author -->
                        <AppTooltip
                            v-if="auth.user && auth.user.permissions.includes('Blog: Author - Edit')"
                            text="Edit Author"
                            class="mr-3"
                        >
                            <AppButton
                        ...
                        <!-- delete author -->
                        <AppTooltip
                            v-if="auth.user && auth.user.permissions.includes('Blog: Author - Delete')"
                            text="Delete Author"
                        >
                            <AppButton
